...
const POPULAR=["Mumbai","Delhi","London","Paris","Tokyo","New York","Dubai","Singapore","Sydney","Berlin","Toronto","Los Angeles","Shanghai","Seoul","Bangkok","Chennai","Kolkata","Bengaluru","Hyderabad","Kozhikode","Kochi","Trivandrum"];
...
function windDirStr(deg){if(deg===undefined)return"--";const d=["N","NE","E","SE","S","SW","W","NW"];return d[Math.round(deg/45)%8];}
...
function renderResult(){
  ...
  // compass
  const needle=document.getElementById("r-compass-needle");
  setTimeout(()=>{needle.style.transform=`rotate(${d.wind.deg||0}deg)`;},200);
  document.getElementById("r-wind-speed").textContent=Math.round(d.wind.speed||0);
  document.getElementById("r-wind-dir").textContent="Direction: "+windDirStr(d.wind.deg);
  if(d.wind.gust) document.getElementById("r-wind-gust").textContent="💨 Gusts up to "+Math.round(d.wind.gust)+" m/s";
...
async function doSearch(city){
  ...
  document.getElementById("search-skeleton").style.display="block";
...
input.addEventListener("input",()=>{
  ...
  sugBox.innerHTML=matches.map(c=>`<div class="sug-item" data-city="${c}"><i class="fa-solid fa-location-dot"></i>${c}</div>`).join("");
  sugBox.querySelectorAll(".sug-item").forEach(el=>{
    el.addEventListener("click",()=>{input.value=el.dataset.city;doSearch(el.dataset.city);});
  });
});
